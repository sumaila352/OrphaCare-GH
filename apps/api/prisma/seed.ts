...
async function main() {
  ...
  const adminEmail = (process.env.SEED_ADMIN_EMAIL?.trim() || 'admin@orphacare.local').toLowerCase();
  const adminPassword = process.env.SEED_ADMIN_PASSWORD?.trim() || 'Admin@123';
  const syncPassword = !isProd || !!process.env.SEED_ADMIN_PASSWORD;
  ...
  console.log(
    `Seed complete. Admin login: ${adminEmail} (id=${admin.id})${syncPassword ? ' — password synced from SEED_ADMIN_PASSWORD' : ''}${seedDemo ? ' + demo data' : ''}`,
  );
  if (isProd && !process.env.SEED_ADMIN_PASSWORD) {
